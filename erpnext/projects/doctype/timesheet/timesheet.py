...
class Timesheet(Document):
	def validate(self):
		...
		self.calculate_percentage_purchased()
		self.set_dates()
	...
	def calculate_total_amounts(self):
		...
		# NEW
		self.total_purchasable_hours = 0.0
		self.total_purchased_hours = 0.0
		self.total_purchasable_amount = 0.0
		self.total_purchased_amount = 0.0
		...
		for d in self.get("time_logs"):
			...
			# NEW
			self.total_purchasable_hours += flt(d.hours)
			self.total_purchasable_amount += flt(d.costing_amount)
			self.total_purchased_amount += flt(d.costing_amount) if d.purchase_invoice else 0.0
			self.total_purchased_hours += flt(d.hours) if d.purchase_invoice else 0.0
	...
	# NEW -------------------------------------------------
	def calculate_percentage_purchased(self):
		self.per_purchased = 0
		if self.total_purchased_amount > 0 and self.total_purchasable_amount > 0:
			self.per_purchased = (self.total_purchased_amount * 100) / self.total_purchasable_amount

	#def timesheet_check(self):
		#saved_doc = frappe.get_doc('Timesheet', self.name)
		#if saved_doc.time_logs != self.time_logs:
			#for time in saved_doc.time_logs:
				#frappe.msgprint(_("saved_doc.time_logs {0}").format(time.name))
			#for time in self.time_logs:
				#frappe.msgprint(_("self.time_logs {0}").format(time.name))
	#------------------------------------------------------
	...
	def update_cost(self):
		for data in self.time_logs:
			# NEW -------
			if data.project and self.supplier:
				data.billable = 1
				project = frappe.get_doc('Project', data.project)
				for project_rate in project.supplier_rates:
					if project_rate.supplier == self.supplier:
						supplier_rate = project_rate

				costing_rate = supplier_rate.service_rate_day or 0
				if data.from_time:
					date = get_datetime(data.from_time)
					hour = date.hour
					if hour < 8 or hour >= 22:
						costing_rate = supplier_rate.service_rate_night or 0
					if date.weekday() == 5 or date.weekday() == 6:
						costing_rate = supplier_rate.service_rate_weekend or 0

				costing_hours = data.hours or 0
				data.costing_hours = costing_hours
				data.costing_rate = costing_rate
				data.costing_amount = costing_rate * costing_hours
...
def query_condition_for_timesheet(arg):
	if 'Projects Manager' in frappe.get_roles(frappe.session.user):
		return """(`tabTimesheet`.name in (SELECT name FROM `tabTimesheet`))"""
		#timesheets = frappe.get_all("Timesheet", fields=["name"])
		#if len(timesheets) > 0:
		#	timesheet_names = [timesheet.name for timesheet in timesheets]
		#	query_names = ', '.join("'{0}'".format(name) for name in timesheet_names)
		#	#query_names = "'TS-2020-00010'"
		#	print('query_names {}'.format(query_names))
		#	return "(`tabTimesheet`.name in ({names}))".format(names=query_names)
	else:
		return None
