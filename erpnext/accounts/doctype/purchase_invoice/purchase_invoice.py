...
class PurchaseInvoice(BuyingController):
	...
	def validate(self):
		...
		self.set_costing_hours_and_amount()
		self.calculate_costing_amount_for_timesheet()
		self.calculate_costing_amount_for_appointments()

	# New
	def on_update(self):
		self.update_time_sheets(self.name)
		self.update_appointments(self.name)

	# New
	def on_trash(self):
		self.update_time_sheets(None)
		self.update_appointments(None)
	...
	def on_submit(self):
		...
		# NEW SUBMIT - TIMESHEETS and Appointments
		self.update_time_sheets(self.name)
		self.update_appointments(self.name)
	...
	# ------------------ NEW - Purchase Invoice Time Sheets and Appointments  -----
	...
	def set_costing_hours_and_amount(self):
		for timesheet in self.timesheets:
			ts_doc = frappe.get_doc('Timesheet', timesheet.time_sheet)
			if not timesheet.costing_hours and ts_doc.total_purchasable_hours:
				timesheet.costing_hours = ts_doc.total_purchasable_hours

			if not timesheet.costing_amount and ts_doc.total_purchasable_amount:
				timesheet.costing_amount = ts_doc.total_purchasable_amount

		for entry in self.appointments:
			ap_doc = frappe.get_doc('Patient Appointment', entry.appointment)
			if ap_doc.price:
				entry.costing_amount = ap_doc.price

	def calculate_costing_amount_for_timesheet(self):
		total_costing_amount = 0.0
		for data in self.timesheets:
			if data.costing_amount:
				total_costing_amount += data.costing_amount

		self.total_costing_amount = total_costing_amount

	def calculate_costing_amount_for_appointments(self):
		total_costing_amount_appointments = 0.0
		for data in self.appointments:
			if data.costing_amount:
				total_costing_amount_appointments += data.costing_amount

		self.total_costing_amount_appointments = total_costing_amount_appointments

	def update_time_sheets(self, purchase_invoice):
	...
	def update_time_sheet_detail(self, timesheet, args, purchase_invoice):
		for data in timesheet.time_logs:
			if (not data.purchase_invoice) or \
				(not purchase_invoice and data.purchase_invoice == self.name):
				data.purchase_invoice = purchase_invoice

	def update_appointments(self, purchase_invoice, submit=False):
		for ap in self.appointments:
			if ap.appointment:
				appointment = frappe.get_doc("Patient Appointment", ap.appointment)
				appointment.flags.ignore_validate_update_after_submit = True
				appointment.purchase_invoice = purchase_invoice
				appointment.save()

	# ----------------- NEW - Purchase Invoice Time Sheets -----
...
def edit_bulk_status(names, status, mode_of_payment=None):
	...
	if status not in field_status.options:
		throw(_("Invalid Status option {0}").format(status))
	for name in names:
